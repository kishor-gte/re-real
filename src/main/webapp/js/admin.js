...
(function (global) {
  ...
  const AdminPgOwnerAPI = {
    base: '',
    async request(url, options) {
      options = options || {};
      const fullUrl = url.startsWith('http') ? url : (AdminPgOwnerAPI.base || '') + url;
      const res = await fetch(fullUrl, Object.assign({ credentials: 'same-origin' }, options, {
        headers: Object.assign({ 'Content-Type': 'application/json' }, options.headers || {})
      }));
      const data = await res.json().catch(function () { return {}; });
    ...
    list(status) {
      return this.request('/api/admin/pg-owners?status=' + encodeURIComponent(status || 'PENDING'));
    },
    pendingCount() {
      return this.request('/api/admin/pg-owners/pending-count');
    },
    get(id) {
      return this.request('/api/admin/pg-owners/' + id);
    },
    approve(id) {
      return this.request('/api/admin/pg-owners/' + id + '/approve', { method: 'POST', body: '{}' });
    },
    reject(id, reason) {
      return this.request('/api/admin/pg-owners/' + id + '/reject', {
        method: 'POST',
        body: JSON.stringify({ reason: reason })
      });
    },
    suspend(id) {
      return this.request('/api/admin/pg-owners/' + id + '/suspend', { method: 'POST', body: '{}' });
    },
    reactivate(id) {
      return this.request('/api/admin/pg-owners/' + id + '/reactivate', { method: 'POST', body: '{}' });
    }
  };
  ...
  const AdminUserAPI = {
    ...
    async request(url, options) {
      options = options || {};
      const fullUrl = url.startsWith('http') ? url : (AdminUserAPI.base || '') + url;
      const res = await fetch(fullUrl, Object.assign({ credentials: 'same-origin' }, options, {
        headers: Object.assign({ 'Content-Type': 'application/json' }, options.headers || {})
      }));
      const data = await res.json().catch(function () { return {}; });
    ...
    list(role, status, q) {
      let url = '/api/admin/users?role=' + encodeURIComponent(role || 'ALL') + '&status=' + encodeURIComponent(status || 'ALL');
      if (q) url += '&q=' + encodeURIComponent(q);
      return this.request(url);
    }
  };

  const AdminPropertyAPI = {
    base: '',
    async request(url, options) {
      options = options || {};
      const fullUrl = url.startsWith('http') ? url : (AdminPropertyAPI.base || '') + url;
      const res = await fetch(fullUrl, Object.assign({ credentials: 'same-origin' }, options, {
        headers: Object.assign({ 'Content-Type': 'application/json' }, options.headers || {})
      }));
      const data = await res.json().catch(function () { return {}; });
    ...
    listByAgent(status, q) {
      let url = '/api/admin/properties?status=' + encodeURIComponent(status || 'ALL');
      if (q) url += '&q=' + encodeURIComponent(q);
      return this.request(url);
    }
  };
  ...
  global.AdminPgOwnerAPI = AdminPgOwnerAPI;
  global.AdminUserAPI = AdminUserAPI;
  global.AdminPropertyAPI = AdminPropertyAPI;
